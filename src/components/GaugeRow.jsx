...
export default function GaugeRow() {
  ...
  return (
    <div className="scada-panel" style={{ padding: '12px 14px', display: 'flex', justifyContent: 'space-around', flexWrap: 'wrap', gap: 10 }}>
      ...
      <Gauge label="HUMEDAD PROM." value={avgHumRaw} unit="%" min={0} max={14} setpoint={TML} color={avgHumRaw != null ? humColor(avgHumRaw) : '#1a73e8'} />
      <Gauge label="LEY DE FE PROM." value={avgFeRaw} unit="%" min={60} max={70} setpoint={65.5} color="#1f9d55" />
    </div>
  );
}
